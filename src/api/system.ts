...
import { PageResult, Result } from "@/api/utils";

// ***********************系统配置管理***********************
export const getSystemConfigPage = (data?: object) => {
  return http.request<PageResult<any>>("post", "/system/config/page", { data });
};
export const saveSystemConfig = (data?: object) => {
  return http.request<Result<any>>("post", "/system/config/save", { data });
};
export const updateSystemConfig = (data?: object) => {
  return http.request<Result<any>>("post", "/system/config/update", { data });
};
export const updateSystemConfigValue = (data?: object) => {
  return http.request<Result<any>>("post", "/system/config/updateValue", { data });
};
export const deleteSystemConfig = (id: number) => {
  return http.request<Result<any>>("get", "/system/config/delete" + id);
};
// ***********************系统配置管理***********************
